...
3. **Relationship to existing PinGarden methods** — how the framework differs from or complements `portfolio-map`, `business-model-environment`, `innovation-metrics`, `scenario-planning`, or other existing frameworks.
4. **Canvas mapping** — for every `relatedCanvasDefIds[]` entry, explain how the framework turns into canvas objects, zones, pins, stickies, axes, or interpretation rules.
5. **Usage workflow** — at least 5–7 practical steps covering inputs, canvas filling, interpretation, decision output, and next action.
6. **Result interpretation** — how to read common positions, shapes, quadrants, clusters, gaps, or signals.
7. **Example cases** — a `What to notice in examples` section explaining why each example is a real teaching case.
8. **Common misuses** — at least 4–6 method-specific anti-patterns.
9. **Authority sources** — official or primary sources where available. Use McKinsey, BCG, Bain, HBR/HBS, World Bank, OECD, WIPO, or comparable sources as primary citations.
10. **Operational skill guidance** — `skill.*.md` must include selection criteria, fill order, canvas mapping, questions to ask, and red flags.
...
## 5. Portfolio-management family relationship
