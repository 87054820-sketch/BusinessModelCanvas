...
> A company creates and captures value by systematically collaborating with outside partners. Two structural sub-types per Chesbrough: outside-in opens R&D to external ideas inside the firm (P&G Connect & Develop), inside-out monetizes idle internal IP through external parties (GlaxoSmithKline patent pools).
...
This pattern has 2 structurally distinct sub-types — picking the right one is the whole game (walk the decision tree in `description.en.md`).

### Outside-in (external ideas in)

The company opens its R&D / product process to systematically acquire external ideas, technology, or IP — through scouts, partnerships, crowdsourcing platforms, or licensing-in. Search cost falls; the firm trades 'NIH' (Not Invented Here) for productivity.

**Examples**: `procter-gamble-cd` (primary), `innocentive` (secondary)

### Inside-out (idle IP out)

The company monetizes idle internal IP, R&D outputs, or technology by licensing, spin-out, joint-venture, or pool — capturing value from assets it would not commercialize itself. Cannibalization fear is the main barrier.

**Examples**: `glaxosmithkline-patent-pool` (primary)
...
- **Chesbrough 2006** · *Henry Chesbrough · Open Business Models: How to Thrive in the New Innovation Landscape · Harvard Business Review Press* · 2006
  Book-length expansion that codified the outside-in vs inside-out distinction now used across the field. Argues the business model — not just the R&D process — has to be open, otherwise external ideas die in commercialization.

- **BMG 2010** · *Osterwalder & Pigneur · Business Model Generation · Wiley* · 2010 · pp. 108–119
  Adapts Open into the BMC pattern catalog (Pattern No. 5). Uses P&G Connect & Develop as the outside-in paradigm and GlaxoSmithKline patent pools as the inside-out paradigm; covers InnoCentive as the connector platform that runs the pattern as-a-service for many seekers.
...
- **Chesbrough 2003 MIT Sloan** · *[Henry Chesbrough · 'The Era of Open Innovation' · MIT Sloan Management Review](https://sloanreview.mit.edu/article/the-era-of-open-innovation/)* · 2003 · Vol. 44 No. 3, Spring 2003
  The originating article that named 'open innovation'. Contrasts the closed-innovation paradigm (smart people work for us; we win by doing the best internal R&D) with an open paradigm where external knowledge and unused internal IP both create value if their flows are designed for.
